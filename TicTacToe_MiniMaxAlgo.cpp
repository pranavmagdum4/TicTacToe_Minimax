...
void updateStatistics(int winner)
{
    ofstream statsFile("statistics.txt", ios::app);
    if (statsFile.is_open())
    {
        if (winner == COMPUTER)
            statsFile << "Computer Wins\n";
        else if (winner == HUMAN)
            statsFile << "Human Wins\n";
        else
            statsFile << "Draw\n";
        statsFile.close();
    }
...
void playTicTacToe(int whoseTurn)
{
    ...
    {
        printf("It's a draw\n");
        updateStatistics(0); // 0 represents a draw
        displayStatistics();
    }
    ...
    else
    {
        if (whoseTurn == COMPUTER)
        {
            whoseTurn = HUMAN;
            cout << "HUMAN has won\n";
            updateStatistics(HUMAN);
        }
        else if (whoseTurn == HUMAN)
        {
            whoseTurn = COMPUTER;
            cout << "COMPUTER has won\n";
            updateStatistics(COMPUTER);
        }
        displayStatistics();
